...
- `/game` runtime state lives in `useGameStore`. A judgment earns its correct-decision score only after the manual evidence, required physical calculator step, and case-specific Ramp controls are complete. Presentation-only action requirements live beside the case presentation in `gameData`; the receipt catalog remains the source for transaction facts and expected decisions. `LabScene` connects the physical calculator, freeze control, and stamps to that same store and must gate them while paused or after feedback is recorded.
