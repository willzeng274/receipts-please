...
- a complete game-grade Expense OS workflow with six manual cases and five Ramp cases, evidence-backed decisions, a progressing queue, timer/scoring, case-scoped notifications and card controls, local session persistence, and the manual-to-Ramp migration;
...
The embedded workstation Expense OS is the lab counterpart to that game: it carries six manual cases and five Ramp cases with evidence-backed decisions, timer/scoring, local session persistence, and the manual-to-Ramp migration. Keep real authentication, live finance-provider integrations, freeform physical document handling, organic employee characters, and branching dialogue deferred.
...
The approved game midpoint is now a one-click automation payoff. `/` mounts the full shared **Expense OS**, not a separate simplified game imitation, so its app dock, evidence scratchpad, notifications, audit trail, queue, physical stamps, and card controls all use `useWorkstationStore`. The manual OS remains dark, fragmented, and stressful through all seven manual cases, with cortisol starting at 28 percent and rising on every workstation click or key event plus larger receipt and system-effect spikes. It then presents one prominent top-left action: **Install Ramp**. A persistent top-right **Manual / Post-Ramp** switch also makes the Post-Ramp sequence available from every game state for demos; switching back resets to the manual opening. Activating either Ramp entry exits monitor focus, shakes the camera/world, and automatically advances the existing six migration stages. Ramp then completes the surfaced evidence, control actions, decisions, six-case exception queue, and automatic-decline ending while a visible status surface reports progress and conflicting manual inputs remain locked. The office still changes from dark/manual lighting to bright/Ramp lighting, cortisol drops to 22 percent, and the supplied `control-desk/public/brand/low-cortisol.jpeg` image briefly fades in and out. The CEO decline dialogue must remain on screen until the player clicks to continue; only then may the service-window giraffe reveal begin. It is a payoff to the established manual workflow, never an opening ad.
...
- During the manual phase, reveal the six canonical submissions over a deterministic pseudo-random schedule while the focused session is running. Each arrival uses the prototype Slack ping at high volume, an interrupting on-screen alert, an unread notification, a Slack feed entry, and the shared `receipt.submitted` paper-drop event; stop this scheduler and any active alert audio during migration and Ramp mode.
...
- Workstation focus uses the authored seated eye point and keeps Expense OS mounted only on the physical monitor; never replace it with a floating screen-space panel. Keep the right decision wing visible and raycastable; the physical APPROVE, REJECT, and FRAUD stamps submit `approve`, `reject`, and `investigate` through the same evidence validation, score, audit, and scene-effect path as OS controls.
