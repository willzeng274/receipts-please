...
- Expense OS case state lives in `useWorkstationStore`: active case/app, submitted and closed case IDs, pinned evidence, immutable audit snapshots, per-card state, score/timer state, notifications, and semantic scene-event IDs remain serializable. Persist only this sanitized game session under a versioned local-storage key. `useLabStore` continues to own lab mode, focus/camera, lighting, shared effect runs, and migration phase. `LabScene` is the only bridge that maps workstation scene events onto shared effect intents; workstation components never manipulate Three refs.
