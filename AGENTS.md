...
- `/game` as the user-facing vertical slice: six manual cases, six automatically handled Ramp exceptions, physical desk tools, decisions, required actions, timing, scoring, one-click migration, adaptive prototype audio, and the giraffe ending.
...
The approved `/game` midpoint is now a one-click automation payoff. `/game` mounts the full shared **Expense OS**, not a separate simplified game imitation, so its app dock, evidence scratchpad, notifications, audit trail, queue, physical stamps, and card controls all use `useWorkstationStore`. The manual OS remains dark, fragmented, and stressful through all six manual cases, with cortisol starting at 28 percent and rising on every workstation click or key event plus larger receipt and system-effect spikes. It then presents one prominent top-left action: **Install Ramp**. A persistent top-right **Manual / Post-Ramp** switch also makes the Post-Ramp sequence available from every `/game` state for demos; switching back resets to the manual opening. Activating either Ramp entry exits monitor focus, shakes the camera/world, and automatically advances the existing six migration stages. Ramp then completes the surfaced evidence, control actions, decisions, six-case exception queue, and automatic-decline ending over roughly 30 seconds while a visible status surface reports progress and conflicting manual inputs remain locked. The office still changes from dark/manual lighting to bright/Ramp lighting, cortisol drops to 22 percent, and the supplied `control-desk/public/brand/low-cortisol.jpeg` image briefly fades in and out. It is a payoff to the established manual workflow, never an opening ad.
