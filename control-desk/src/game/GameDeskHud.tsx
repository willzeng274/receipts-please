...
export function GameDeskHud() {
  ...
  if (feedback) {
    ...
    return (
      <section aria-live="polite" className={`game-desk-feedback ${feedback.correct ? 'is-correct' : 'is-wrong'}`}>
        <span>{feedback.correct ? 'JUDGMENT RECORDED' : 'AUDIT NOTE'}</span>
  ...
  return (
    <aside className={`game-desk-hud game-desk-hud--${phase}`}>
      <span>{phase === 'manual' ? 'PHYSICAL DESK · MANUAL SHIFT' : 'PHYSICAL DESK · EXCEPTION CONTROL'}</span>
      <h2>{currentCase.title}</h2>
      <p>{decisions.length === 0
        ? 'The monitor has the records. The calculator and decision stamps are real tools on your desk.'
        : phase === 'manual'
          ? 'Open every relevant record, then return here to calculate and stamp.'
          : 'Ramp prepared the exception. Take any control actions, then make the final judgment here.'}</p>
      <ol>
        ...
        <li><b>{calculatorRequired || requiredActions.length ? '4' : '2'}</b><span>Click Approve, Reject, or Investigate stamp</span></li>
      </ol>
      <button onClick={() => {
        setFocused(true)
